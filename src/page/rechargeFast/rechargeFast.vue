<template>
  <div class="rechargeFast">
    ...
    <div class="rechargeFast-enter" @click='rechargeFastSubmit' :style="{background:payClick?'#ff654f':'#999'}">确定</div>
  </div>
</template>
...
import { requestSmsCodeJson, deductPayRequest } from '@/server/getData.js'
...
export default {
  ...
  methods:{
    getCode(){
      ...
      requestSmsCodeJson(token=getStore('token'), userId=getStore('userId'), amount=this.$route.query.payMoney, smsType=4).then(res => { 
    ...
    rechargeFastSubmit(){
      ...
      this.payClick = false;
      let token, userId, amount, mobileCode, platForm;
      deductPayRequest(token=getStore('token'), userId=getStore('userId'), amount=this.$route.query.payMoney, mobileCode=this.SMSCode, platForm=3).then(res => { 
        if(res.data.code==1){
          this.$refs.windowTips.toast('充值成功');
          this.payClick = true;
          setTimeout(() => {
