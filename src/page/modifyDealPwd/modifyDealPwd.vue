<template>
  <div class="modifyDealPwd">
    <mheader headTitle='修改交易密码' :goBack="'true'" class="borderBottom-1px"></mheader>
    <window-tips ref='windowTips'></window-tips>
    <div class="modifyDealPwd-input">
      <ul>
        <li>
          <label>我的手机号码</label>
        ...
        <li>
          <label>新交易密码</label>
          <div  class="borderBottom-1px">
            <input @keyup.13="modifyDealPwdSubmit" :type="showNewDealPwd?'text':'password'" placeholder="请输入新交易密码" v-model='newDealPwd' autocomplete="off" onkeyup="this.value=this.value.replace(/^ +| +$/g,'')" maxlength="16">
    ...
    <div class="modifyDealPwd-enter" @click='modifyDealPwdSubmit'>确定</div>
  </div>
</template>
...
import { SendSMS, setNewDealPasswordByPhoneNew, textCode } from '@/server/getData.js'
...
export default {
  ...
  methods:{
    getImgCode(){
      this.imgCodeSrc = `${textCode}?pageId=wapModifyPwd&d=${(new Date()).getTime().toString()}`;
    },
    getCode(){
      if(!this.imgCode) {
      ...
      SendSMS(token=getStore('token'), userId=getStore('userId'), phone, send_type='modify_pwd', pageId='wapModifyPwd', imgCode=this.imgCode).then(res => { 
    ...
    modifyDealPwdSubmit(){
      ...
      if(!this.newDealPwd){
        this.$refs.windowTips.toast('请输入新的交易密码');
        return;
      }
      let token, userId, cellphone, send_type, checkCode,newPassword;
      setNewDealPasswordByPhoneNew(token=getStore('token'), userId=getStore('userId'), cellphone, send_type='modify_pwd', checkCode=this.SMSCode, newPassword=encryptByDES(this.newDealPwd)).then(res => { 
        if(res.data.code==1){
          this.$refs.windowTips.toast('交易密码设置成功');
          setTimeout(() => {
